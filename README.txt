...
1. rankedftw.com에 어떤 아이디를 검색을 하면 rankedftw는 그 문자열을 포함하는 모든 아이디들을 검색하여 결과를 내놓지만, 이 코드는 rankedftw의 검색페이지의 첫 페이지만을 크롤링한다. 그렇기 때문에 첫 페이지에 검색한 아이디가 없을 경우 에러가 발생하여 빌드추천 웹사이트를 다시 로드하여야 한다.

2. 상대가 래더를 하지 않거나 매치 전적이 없을 경우 프로필 API에 해당 정보가 비어있어서 다음 request에 사용할 url을 구성하지 못하고 에러가 발생한다. 

3. 원인은 잘 모르지만, localhost로 접속한 웹사이트에서는 검색이 원활이 되는 유저아이디가 aws ip로 접속한 웹사이트에서는 에러를 발생시키는 경우가 있다. ex) Rogue(이병렬 선수)

4. aws에서도 잘 동작하는 테스트용 ID : Stats(김대엽 선수), rogue(일반인)

5. 한글아이디 검색은 구현하지 않아서 불가능하다.